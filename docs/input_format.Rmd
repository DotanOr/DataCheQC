...
The format can handle various types of data, such as demographics, pharmacokinetics (PK), and pharmacodynamics (PD), and requires minimal manipulation of [Clinical Data Interchange Standards Consortium (CDISC)](http://www.cdisc.org) data to compile a resulting dataset. The generalized dataset can be seamlessly converted to an NLME dataset suitable for analysis in [NONMEM](https://nonmem.iconplc.com/#/nonmem743/guides) and [Monolix](http://lixoft.com/products/monolix) using available tools.
  
The high rate of success and efficiency that were achieved while working with the generalized dataset across different drug development programs and organizations influenced the choice of this particular format. Requirements for the datasets to be used with QC app are detailed in the table below:
...
htmltools::br()
htmltools::tags$a(class = "example-link", href="https://raw.githubusercontent.com/DotanOr/DataCheQC/main/Examples/Example_Dataset.csv", target="_blank", "Click here to download an example dataset in this format")
```
...
htmltools::br()
htmltools::tags$a(class = "example-link", href="https://raw.githubusercontent.com/DotanOr/DataCheQC/main/Examples/Example_Specification_File.docx", target="_blank", "Click here to download an example of a full specification file containing both tables.")
```
